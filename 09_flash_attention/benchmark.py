...
BATCH, N_HEADS, HEAD_DIM = 32, 4, 128 # LOWER THESE IF YOU DON'T HAVE ENOUGH RAM
...
def bench_flash_attention(BATCH, H, SEQ_LEN, HEAD_DIM, mode, provider, device=DEVICE):
    ...
    if provider == 'original_paper':
        fn = lambda: original_paper.apply(q, k, v, True, sm_scale)
    if provider == 'umar':
        fn = lambda: umar.apply(q, k, v, True, sm_scale)
    if provider == 'triton_docs':
        fn = lambda: triton_docs.apply(q, k, v, True, sm_scale)
